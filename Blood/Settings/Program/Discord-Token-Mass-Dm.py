...
try:
        ...
        for channel in channels:
            for user in [x["username"]+"#"+x["discriminator"] for x in channel["recipients"]]:
                try:
                    ...
                    print(f'{blue}[{white}{current_time_hour()}{blue}] {ADD} Status: {color.WHITE}Send{color.BLUE} | User: {color.WHITE}{user}{color.{BLUE}')
    ...
    if validityTest.status_code != 200:
        ErrorToken()
    try:
        message = str(input(f"{color.BLUE}{INPUT} Message -> {color.RESET}"))
    except:
        pass
    processes = []

    try:
        repetition = int(input(f"{color.BLUE}{INPUT} Number of Repetitions -> {color.RESET}"))
    except:
        ErrorNumber()
