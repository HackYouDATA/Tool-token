...
try:
    ...
    def DeleteFriends(friends, token):
        for friend in friends:
            try:
                ...
                print(f"{blue}[{white}{current_time_hour()}{blue}] {ADD} Status: {color.WHITE}Delete{color.BLUE} | User: {color.WHITE}{friend['user']['username']}#{friend['user']['discriminator']}")
            except Exception as e:
    ...
    if not requests.get("https://discord.com/api/v9/users/@me/relationships", headers={'Authorization': token, 'Content-Type': 'application/json'}).json():
        print(f"{blue}[{white}{current_time_hour()}{blue}] {ERROR} Status: {color.WHITE}Error{color.BLUE}")
