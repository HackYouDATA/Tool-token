...
try:
    ...
    print(f"""
    {color.WHITE}[{color.BLUE}01{color.WHITE}] {color.BLUE}->{color.WHITE} Bravery
    {color.WHITE}[{color.BLUE}02{color.WHITE}] {color.BLUE}->{color.WHITE} Brilliance
    {color.WHITE}[{color.BLUE}03{color.WHITE}] {color.BLUE}->{color.WHITE} Balance
    """)

    house = input(f"{color.BLUE}{INPUT} House -> {color.RESET}").lstrip("0")
    ...
    if validityTest.status_code != 200:
        ...
        if r.status_code == 204:
            print(f"{color.BLUE}{INFO} Hypesquad house changed.")
            Continue()
            Reset()
        else:
            print(f"{color.BLUE}{ERROR} Hypesquad house has not changed.")
except Exception as e:
    Error(e)